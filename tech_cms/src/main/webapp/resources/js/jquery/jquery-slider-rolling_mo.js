...
(function($) {
  ...
  $.fn.srolling_mo = function(settings) {
...
//    var delay_time = delay;
    ...
    var box_area = box.append("<div id='" + str_name + "' style='width:" + w_full_size+start_left + "px; height:90%; position: absolute;white-space:nowrap;'></div>");
    box_area = jQuery(name,box);
    var item_area = jQuery("<div></div>").css("width",item_width/*30+'%'*/).css("height",item_height).css("float","left").css("overflow","hidden").css("margin-left","0.1%");
    ...
    function _start_box() {
      switch (move) {
        ...
        case 'right' : 
        case 'left' : box_area.css('left',0); break;
      }
    }
    ...
    function _items() {
      ...
      for (var i = cache_count; i > 0; i--) {
        ...
        box_area.prepend( t_obj.append(jQuery(data[prev_idx])).attr('class','srolling_item_' + prev_idx + ' dt') );
        prev_idx--;
      }
      ...
      for (var i=0; i < cache_count; i++) {
        ...
        var t_obj = item_area.clone();
        box_area.append( t_obj.append(jQuery(data[next_idx])).attr('class','srolling_item_' + next_idx + ' dt') );
        next_idx++;
      }
