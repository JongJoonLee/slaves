	...
	<div class="col-md-10">
		...
		<table class="table table-striped table-condensed">
			...
			<c:forEach var="tmp" items="${list}">
				<tr>
					<td style="text-align: center">${tmp.tb_promote_no}</td>
					...
					<td style="text-align: center">${tmp.tb_promote_hit}</td>

					<td style="text-align: center"><c:set var="dates"
							value="${fn:split(tmp.tb_promote_moddate,' ')}" /> <c:out
							value="${dates[0]}" /></td>
				</tr>
			</c:forEach>
		</table>
			...
			<tr>
				...
				<td width="20%" align="right" colspan="2"><c:set var="session"
						...
						<c:when test="${session eq null}">
							<a href="${pageContext.request.contextPath}/cms/promote_write.do"
								class="btn btn-primary"> 글작성 </a>
						</c:when>
						<c:otherwise>
							<a href="${pageContext.request.contextPath}/cms/promote_write.do" class="btn btn-primary"> 글작성 </a>
