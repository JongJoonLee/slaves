...
//    var delay_time = delay;
    ...
    var box_area = box.append("<div id='" + str_name + "' style='position: absolute;white-space:nowrap; border: 0px;'></div>");
    ...
    function _prev_act() {
      ...
      switch (move) {
        case 'left' : 
        case 'right' : 
          _animate({ left: '+=' + (item_width * item_count) },item_count,'prev', tempP);
        break;

        case 'up' : 
        case 'down' : 
          _animate({ top: '+=' + (item_height * item_count) },item_count,'prev', tempP);
        break;
    ...
    function _next_act() {
      ...
      switch (move) {
        case 'down' : 
        case 'up' : 
        _animate({ top: '-=' + (item_height * item_count) },item_count, move, tempN);
        break;

        case 'right' : 
        case 'left' : 
        _animate({ left: '-=' + (item_width * item_count) },item_count, move, tempN);
        break;
      }
    ...
    function _animate(get_item,move_idx,act,pbflag) {
      box_area.animate(
      get_item
