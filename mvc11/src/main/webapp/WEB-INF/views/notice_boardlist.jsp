						...
						<tr>
							...
							<td><c:choose>
									<c:when test="${fn:length(tmp.getTitle())>20}">
										<a href="notice_boardcontent.do?no=${tmp.getNo()}"><c:out
												value="${fn:substring(tmp.getTitle(),0,20)}" />...</a>
									</c:when>
									<c:otherwise>
										<a href="notice_boardcontent.do?no=${tmp.getNo()}">${tmp.getTitle()}</a>
									</c:otherwise>
								</c:choose></td>
					...
					<tr>
						...
						<td width="20%" align="right" colspan="2"><c:set
								...
								<c:when test="${session eq null}">
									<a href="login.do?prev=notice_boardwrite.do" class="btn btn-primary">
										글작성 </a>
								</c:when>
								<c:otherwise>
