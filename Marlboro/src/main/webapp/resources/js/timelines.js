...
$(function() {
	...
	$("#commentParentSubmit").click(
			function(event) {
				...
				var pPassword = $("#commentParentPassword");*/// 패스워드를 노출 시켰는데
															// 저장하고 나서 저장한 날짜를
															// 보여줄 예정
...
/*
				...
				/*var commentParentText = '<tr id="r1" name="commentParentCode">'
						+ '<td colspan=2>' + '<strong>' + pName.val()
						+ '</strong> ' + pPassword.val()
						+ '<a style="cursor:pointer;" name="pDel">삭제</a><p>'
						+ pText.val().replace(/\n/g, "<br>") + '</p>' + '</td>'
						+ '</tr>';*/

				var commentParentText = 
					'<div class="message-item"> <div class="message-inner"> <div class="message-head clearfix"> <div class="avatar pull-left">'
					+'<a href=""><img class="img1" src="https://ssl.gstatic.com/accounts/ui/avatar_2x.png"></a>'
					+'</div><div class="user-detail"><h5 class="handle">${tmp.getMem_name()} ${tmp.getMem_grade()} / ${tmp.getMem_team()}</h5>'
					+'<div class="post-meta"><div class="asker-meta"><span class="qa-message-what"></span> <span class="qa-message-when">'
					+'<span class="qa-message-when-data">${tmp.getTime_date()}</span></span></div>'
					+'</div></div></div><table id="commentTable" class="table table-condensed"></table>'
					+'<div id="commentTable" class="qa-message-content">'
					+pText.val().replace(/\n/g, "<br>") + '</p>'
					+'</div></div>';
			...
			.on(
					...
					function() {
						...
						var commentChildText = '<tr name="commentChildCode">'
								...
								+ '</strong> '
								+ cPassword.val()
								+ '<a style="cursor:pointer;" name="cDel">삭제</a>'
								+ '<p>' + cText.val().replace(/\n/g, "<br>")
								+ '</p>' + '</td>' + '</tr>';
